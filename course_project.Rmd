...
pml_testing_data  <- read.csv("data/pml-training.csv", na.strings=c("NA","#DIV/0!",""), stringsAsFactors = TRUE)
```
## Clean the Data
...
```{r}
inTrain <- createDataPartition(y=trainingData$classe, p=0.08, list=FALSE)
...
testPredictions <- predict(rfFit, newdata = scaledTestSubmission)
```